...
const Emitter = new NativeEventEmitter(BackgroundTimer);

// 时钟池
let TimerPool = [];

/**
 * 对原生BackgroundTimer的封装类
 */
class BgTimer {
    /**
     * 倒计时
    ...
    timer(interval, count, fn, finalcb){
        ...
        BackgroundTimer.timer(timer.eventName, interval, count);
    ...
    clear(eventName){
        //调用原生方法
        BackgroundTimer.clear(eventName);
        //清除时钟池中的时钟信息对象
        for(let i = 0; i < TimerPool.length; i++){
            let ti = TimerPool[i];
     ...
     */
    getEventName() {
        let basicEventName = BackgroundTimer.basicEventName;
        let basicEventNameLen = basicEventName.length;
        // 判断是否已经超过最大限制
        if (TimerPool.length >= BackgroundTimer.maxTimerCount){
            throw new Error('BackgroundTimer时钟数量已经超过最大数量！');
        }
